...
export default function ProfilePage() {
  ...
  return (
    <div style={{ padding: "28px 24px", maxWidth: "760px", margin: "0 auto", display: "flex", flexDirection: "column", gap: "20px" }}>
      ...
      {/* ── Hero card ── */}
      <div style={{ background: "linear-gradient(135deg, #0D0D1A 0%, #131325 100%)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "20px", padding: "24px", position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", top: 0, right: 0, width: "200px", height: "200px", background: "radial-gradient(circle, #7C3AED, transparent)", filter: "blur(48px)", opacity: 0.18, pointerEvents: "none" }} />

        {/* Profile row */}
        <div style={{ position: "relative", zIndex: 1, display: "flex", alignItems: "flex-start", gap: "18px", flexWrap: "wrap" }}>
          <div style={{ position: "relative", flexShrink: 0 }}>
            <div style={{ width: "76px", height: "76px", borderRadius: "50%", background: "linear-gradient(135deg, #7C3AED, #06B6D4)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 900, fontSize: "28px", boxShadow: "0 8px 24px rgba(124,58,237,0.35)" }}>
              {form.name[0]}
            </div>
            <div style={{ position: "absolute", bottom: 3, right: 3, width: "14px", height: "14px", borderRadius: "50%", background: "#4ADE80", border: "2.5px solid #0D0D1A" }} />
          </div>

          <div style={{ flex: 1, minWidth: 0 }}>
            <h1 style={{ color: "#fff", fontWeight: 800, fontSize: "22px", margin: "0 0 4px" }}>{form.name}</h1>
            <p style={{ color: "rgba(255,255,255,0.38)", fontSize: "13px", margin: "0 0 10px" }}>Member since {PROFILE.joined}</p>
          ...
          <button
            ...
            style={{ flexShrink: 0, display: "flex", alignItems: "center", gap: "8px", padding: "10px 18px", borderRadius: "12px", fontSize: "13px", fontWeight: 600, cursor: "pointer", border: "none", background: editing ? "rgba(255,255,255,0.07)" : "linear-gradient(135deg, #7C3AED, #0891B2)", color: "#fff", outline: "none" }}
          >
        ...
        <div style={{ position: "relative", zIndex: 1, display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "12px", marginTop: "20px" }}>
          {[
            ...
            { label: "Certificates",     value: "1",  color: "#34D399" },
            { label: "Achievements",     value: "3",  color: "#FBBF24" },
          ].map(({ label, value, color }) => (
            <div key={label} style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.09)", borderRadius: "12px", padding: "14px 12px", textAlign: "center" }}>
        ...
        {editing ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
            {[
              ...
              <div key={label} style={{ display: "flex", alignItems: "flex-start", gap: "12px", padding: "12px 14px", borderRadius: "10px", background: "rgba(255,255,255,0.02)" }}>
            ...
            <div key={key}>
              ...
              {idx === 2 && pwForm.confirm && (
                <p style={{ fontSize: "12px", display: "flex", alignItems: "center", gap: "5px", color: pwForm.newPw === pwForm.confirm ? "#4ADE80" : "#F87171", marginTop: "6px" }}>
                  {pwForm.newPw === pwForm.confirm ? <CheckCircle size={12} /> : "✗"}{" "}
      ...
      {/* ── Account Status ── */}
      <div style={{ display: "flex", alignItems: "center", gap: "10px", padding: "14px 16px", borderRadius: "12px", background: "rgba(16,185,129,0.05)", border: "1px solid rgba(16,185,129,0.2)" }}>
        <CheckCircle size={16} style={{ color: "#4ADE80", flexShrink: 0 }} />
        <p style={{ fontSize: "13px", color: "#4ADE80", margin: 0 }}>Account verified and active.</p>
      </div>
