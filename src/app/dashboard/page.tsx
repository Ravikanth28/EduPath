...
const RECENT_ACTIVITY = [
  { icon: PlayCircle,  color: "#22D3EE", bg: "rgba(6,182,212,0.1)",    label: "Watched: Introduction to Calculus — Limits",  time: "2 hours ago"  },
  { icon: CheckCircle, color: "#4ADE80", bg: "rgba(74,222,128,0.1)",   label: "Passed: Module 3 Test — 92%",                 time: "5 hours ago"  },
  { icon: Award,       color: "#FCD34D", bg: "rgba(252,211,77,0.1)",   label: "Certificate Earned: Physics Mechanics",        time: "Yesterday"    },
  { icon: BookOpen,    color: "#A78BFA", bg: "rgba(124,58,237,0.1)",   label: "Enrolled: Engineering Mathematics",            time: "2 days ago"   },
];

const COURSES = [
  { id: "1", title: "Engineering Mathematics", category: "Mathematics", progress: 65,  modules: 6, status: "in_progress", grad: "linear-gradient(135deg,#7C3AED,#3B82F6)" },
  { id: "2", title: "Physics Mechanics",       category: "Physics",     progress: 100, modules: 5, status: "completed",   grad: "linear-gradient(135deg,#06B6D4,#0D9488)" },
];
...
const STATS = [
  { icon: BookOpen,    label: "Enrolled Courses",   value: "4",  accent: "#A78BFA", bg: "rgba(124,58,237,0.1)"  },
  { icon: CheckCircle, label: "Completed Modules",  value: "14", accent: "#22D3EE", bg: "rgba(6,182,212,0.1)"   },
  { icon: TrendingUp,  label: "Finished Courses",   value: "1",  accent: "#4ADE80", bg: "rgba(74,222,128,0.1)"  },
  { icon: Award,       label: "Certificates Earned",value: "1",  accent: "#FCD34D", bg: "rgba(252,211,77,0.1)"  },
];
    ...
    <div style={{ padding: "28px 28px 48px", maxWidth: "1200px", margin: "0 auto", display: "flex", flexDirection: "column", gap: "24px" }}>
      ...
      <div style={{ borderRadius: "20px", padding: "28px 32px", position: "relative", overflow: "hidden", background: "linear-gradient(135deg,#0D0D1A,#131325 60%,#0A0A12)", border: "1px solid rgba(124,58,237,0.2)" }}>
        ...
        <div style={{ position: "relative", zIndex: 1, display: "flex", alignItems: "center", justifyContent: "space-between", gap: "20px" }}>
          <div>
            ...
            <h1 style={{ fontSize: "clamp(22px,3vw,32px)", fontWeight: 800, color: "#fff", margin: "0 0 8px", letterSpacing: "-0.5px" }}>
              Welcome back, <span style={{ background: "linear-gradient(135deg,#A78BFA,#22D3EE)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Student</span> 👋
            </h1>
            <p style={{ color: "rgba(255,255,255,0.45)", fontSize: "14px" }}>You have 2 courses in progress. Keep going!</p>
          </div>
            ...
            <div style={{ position: "relative", width: "88px", height: "88px" }}>
              <svg viewBox="0 0 88 88" style={{ width: "100%", height: "100%", transform: "rotate(-90deg)" }}>
                ...
                <circle cx="44" cy="44" r="36" fill="none" strokeWidth="7" strokeLinecap="round"
                  stroke="url(#pg)" strokeDasharray={`${62 * 2.262} ${(100-62)*2.262}`} />
                <defs>
              ...
              <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                <span style={{ fontSize: "17px", fontWeight: 800, color: "#fff" }}>62%</span>
              </div>
            </div>
            <p style={{ fontSize: "11px", color: "rgba(255,255,255,0.35)", marginTop: "6px", textAlign: "center" }}>Overall Progress</p>
          </div>
        </div>
      </div>

      {/* ── Continue learning ────────────────────────────────── */}
      <div style={{ borderRadius: "14px", padding: "16px 20px", display: "flex", alignItems: "center", gap: "16px", position: "relative", overflow: "hidden", background: "linear-gradient(90deg,rgba(124,58,237,0.12),rgba(6,182,212,0.06))", border: "1px solid rgba(124,58,237,0.22)" }}>
        ...
        <Link href="/dashboard/courses/1" style={{ flexShrink: 0, padding: "10px 20px", background: "linear-gradient(135deg,#7C3AED,#06B6D4)", color: "#fff", borderRadius: "10px", fontSize: "13px", fontWeight: 700, textDecoration: "none", display: "flex", alignItems: "center", gap: "7px" }}>
          <PlayCircle size={14} /> Resume
        </Link>
        <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: "3px", background: "linear-gradient(90deg,#7C3AED,#22D3EE)", width: "65%" }} />
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
        ...
        <div style={{ ...card, padding: "22px" }}>
          ...
          <ResponsiveContainer width="100%" height={170}>
            <BarChart data={testScores} margin={{ top: 0, right: 4, bottom: 0, left: -20 }}>
              ...
              <Tooltip
                contentStyle={{ background: "#1A1A30", border: "1px solid rgba(124,58,237,0.3)", borderRadius: "10px", color: "#fff", fontSize: "13px" }}
                cursor={{ fill: "rgba(255,255,255,0.04)" }}
              />
              <Bar dataKey="score" fill="url(#bg)" radius={[6,6,0,0]} />
              <defs>
        ...
        <div style={{ ...card, padding: "22px" }}>
          ...
          {[{ label: "Videos Watched", val: 78, color: "#7C3AED" }, { label: "Tests Passed", val: 92, color: "#22D3EE" }].map(({ label, val, color }) => (
            <div key={label} style={{ marginBottom: "12px" }}>
              ...
              <div style={{ height: "6px", borderRadius: "99px", background: "rgba(255,255,255,0.05)", overflow: "hidden" }}>
                <div style={{ height: "100%", borderRadius: "99px", width: `${val}%`, background: `linear-gradient(90deg, ${color}99, ${color})` }} />
              </div>
            </div>
          ))}
        </div>
            ...
            <div key={c.id} style={{ ...cardHover, overflow: "hidden" }}>
              ...
              <div style={{ padding: "18px 20px" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "10px" }}>
                  ...
                  {c.status === "completed"
                    ? <span style={{ fontSize: "11px", fontWeight: 700, color: "#4ADE80", background: "rgba(74,222,128,0.1)", border: "1px solid rgba(74,222,128,0.25)", borderRadius: "20px", padding: "2px 10px" }}>Done</span>
                    : <span style={{ fontSize: "11px", fontWeight: 700, color: "#22D3EE", background: "rgba(6,182,212,0.1)", border: "1px solid rgba(6,182,212,0.25)", borderRadius: "20px", padding: "2px 10px" }}>{c.progress}%</span>
                  }
                </div>
                ...
                <div style={{ height: "5px", borderRadius: "99px", background: "rgba(255,255,255,0.06)", marginBottom: "12px", overflow: "hidden" }}>
                  <div style={{ height: "100%", borderRadius: "99px", width: `${c.progress}%`, background: "linear-gradient(90deg,#7C3AED,#22D3EE)" }} />
                </div>
      ...
      {CERTS.length > 0 && (
        <div>
          ...
          <div style={{ display: "flex", gap: "14px", overflowX: "auto", paddingBottom: "4px" }}>
            {CERTS.map(c => (
              <div key={c.id} style={{ ...card, padding: "20px", minWidth: "240px", border: "1px solid rgba(252,211,77,0.15)", background: "linear-gradient(135deg,rgba(252,211,77,0.04),rgba(255,255,255,0.02))" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "14px" }}>
                  <Star size={15} color="#FCD34D" />
                  <span style={{ fontSize: "11px", fontWeight: 700, color: "#FCD34D", background: "rgba(252,211,77,0.12)", border: "1px solid rgba(252,211,77,0.25)", borderRadius: "20px", padding: "2px 10px" }}>Earned</span>
                </div>
          ...
          {RECENT_ACTIVITY.map((item, i) => {
            ...
            return (
              <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: "14px", padding: "14px 20px", borderBottom: i < RECENT_ACTIVITY.length - 1 ? "1px solid rgba(255,255,255,0.04)" : "none" }}>
                ...
                <div style={{ flex: 1 }}>
                  <p style={{ fontSize: "14px", color: "rgba(255,255,255,0.82)", marginBottom: "4px", lineHeight: 1.4 }}>{item.label}</p>
                  <p style={{ fontSize: "12px", color: "rgba(255,255,255,0.3)", display: "flex", alignItems: "center", gap: "4px" }}>
