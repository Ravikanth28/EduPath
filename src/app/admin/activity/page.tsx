    ...
    <div className="p-6 space-y-5 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          { label: "Active Today", value: 5, icon: Users, color: "text-cyan-400" },
          { label: "Online Last 30m", value: 2, icon: Clock, color: "text-green-400" },
          { label: "Certs Today", value: 1, icon: Award, color: "text-amber-400" },
      ...
      <div className="glass-card divide-y divide-white/[0.04]">
        {paginated.map(e => (
          <div key={e.id} className="flex items-start gap-4 px-5 py-4 hover:bg-white/[0.02] transition-colors">
            <div className={`w-9 h-9 rounded-xl ${TYPE_BG[e.type]} flex items-center justify-center flex-shrink-0`}>
              {TYPE_ICON[e.type]}
            </div>
            ...
            <div className="flex-1 min-w-0 ml-1">
              <p className="text-sm text-white/70 truncate">{e.detail}</p>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-3">
          <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className="p-2 rounded-lg border border-white/10 text-white/50 hover:text-white hover:border-white/20 transition-colors disabled:opacity-30">
            <ChevronLeft size={15} />
          </button>
          <span className="text-sm text-white/60">Page {page} of {totalPages}</span>
          <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="p-2 rounded-lg border border-white/10 text-white/50 hover:text-white hover:border-white/20 transition-colors disabled:opacity-30">
