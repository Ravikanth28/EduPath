  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        {editing && (
          <button onClick={() => setEditing(false)} className="p-2 rounded-lg hover:bg-slate-200 text-slate-500 hover:text-slate-900 transition-colors">
            <X size={18} />
          </button>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          <div key={label} className="glass-card p-4 flex items-center gap-3">
            ...
            <div><p className="text-sm font-bold text-slate-900">{value}</p><p className="text-xs text-slate-600">{label}</p></div>
          </div>
        ))}
      </div>
      ...
      {editing && (
        <div className="glass-card p-5 space-y-4 red-border">
          <h3 className="font-semibold text-slate-900 text-sm">Edit Student Info</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[["Full Name", "name", "text"], ["Phone Number", "phone", "tel"], ["Email", "email", "email"], ["New Password", "password", "password"]].map(([label, key, type]) => (
              <div key={key as string}>
                <label className="text-xs text-slate-600 mb-1 block">{label as string}</label>
          ...
          <div className="space-y-2">
            {STUDENT.certificates.map(cert => (
              <div key={cert.id} className="flex items-center gap-3 py-2 border-b border-slate-200 last:border-0">
                ...
                <Link href={`/admin/certificates`} className="text-xs text-indigo-600 hover:text-indigo-500">Manage</Link>
              </div>
            ))}
          </div>
        )}
      ...
      <div className="space-y-3">
        <h3 className="font-semibold text-slate-900 flex items-center gap-2"><BookOpen size={16} className="text-indigo-500" /> Course Enrollments</h3>
        {STUDENT.enrollments.map(en => (
          <div key={en.courseId} className="glass-card overflow-hidden">
            ...
            >
              <div className="flex-1">
                <p className="font-medium text-slate-900">{en.courseName}</p>
                <p className="text-xs text-slate-600 mt-0.5">Enrolled {en.enrolled}</p>
              </div>
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-slate-600">{en.progress}%</span>
                </div>
              </div>
            </button>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-slate-200">
                        <th className="text-left py-2 text-xs text-slate-600 font-semibold">Module</th>
                        <th className="text-center py-2 text-xs text-slate-600 font-semibold">Videos</th>
                        <th className="text-center py-2 text-xs text-slate-600 font-semibold">Test</th>
                        <th className="text-center py-2 text-xs text-slate-600 font-semibold">Score</th>
                      </tr>
                    </thead>
                    <tbody>
                      {en.modules.map(mod => (
                        <tr key={mod.num} className="border-b border-slate-100 last:border-0">
                          <td className="py-3 text-slate-700 pr-4">M{mod.num}: {mod.title}</td>
                          <td className="py-3 text-center">
                            <span className="text-xs text-slate-600">{mod.videosDone}/{mod.videosTotal}</span>
                          </td>
                          <td className="py-3 text-center">
                            {mod.testDone ? <CheckCircle size={14} className="text-green-500 mx-auto" /> : <span className="text-xs text-slate-400">-</span>}
                          </td>
                          <td className="py-3 text-center">
                            {mod.testScore !== null ? (
                              <span className={`text-xs font-bold ${mod.testScore >= 70 ? "text-green-600" : "text-red-600"}`}>{mod.testScore}%</span>
                            ) : (
